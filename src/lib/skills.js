export const skills = [
  {
    title: "Web development:",
    body: "Javascript, Typescript, HTML/CSS, Node.js, TailwindCSS",
  },
  { title: "Scripting:", body: "Python, Ruby, Bash" },
  { title: "Systems programming:", body: "Go, Java, C/C++, Rust" },
  { title: "Functional programming:", body: "Elixir, Racket, Elm" },
  {
    title: "Other programming:",
    body: "Swift, Objective-C, Kotlin, C#",
  },
  {
    title: "Frameworks:",
    body: "React, NextJS, Expo, Svelte, SwiftUI, Spring",
  },
  // { title: "UI/UX:", body: "Designing and implementing user interfaces" },
  {
    title: "Databases:",
    body: "PostgreSQL, Supabase, SQLite, Cloud SQL, PocketBase",
  },
  {
    title: "Networking tools:",
    body: "HAProxy, Nginx, Traefik, Caddy",
  },
  {
    title: "Automation tools:",
    body: "Ansible, Jenkins, Gitlab CI/CD, Github Actions",
  },
  {
    title: "Infrastructure tools:",
    body: "Docker, Packer, Vagrant, Terraform, Kubernetes",
  },
  {
    title: "Cloud platforms and providers:",
    body: "Openshift, Openstack, GCP, AWS",
  },
  { title: "AI tools:", body: "Claude Code, OpenCode, OpenAI Codex" },
  // {
  //     title: "Linux server administration:",
  //     body: "managed environments with 100+ hosts",
  // },
  {
    title: "Architecture & Design:",
    body: "Distributed systems, API design, technical roadmapping, mentorship",
  },
  { title: "Languages:", body: "English, Arabic, Russian" },
  // { body: "Quick to pick up new frameworks and programming languages" },
  // { body: "Creative coding, graphics programming game engine architecture, user interface design, responsive design", },
  // Mention graylog and zabbix
];

// TODO Include "I am comfortable working with XYZ languages..."
